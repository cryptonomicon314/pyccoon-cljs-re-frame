...
(def default-loggers
  {:log       #(.log js/console %)
   ...
   ;; group does not exist  < IE 11
   :group     #(if (.group js/console) (.group js/console %) (.log js/console %))  
   ;; groupEnd does not exist  < IE 11
   :groupEnd  #(when (.groupEnd js/console) (.groupEnd js/console))})
...
(defn set-loggers!
  ...
  [new-loggers]
  (assert (empty?
            (difference (set (keys new-loggers))
                        (set (keys default-loggers))))
...
(defn log [& args]
  ((:log @loggers) (apply str args)))

(defn warn [& args]
  ((:warn @loggers) (apply str args)))

(defn group [& args]
  ((:group @loggers) (apply str args)))

(defn groupEnd [& args]
  ((:groupEnd @loggers) (apply str args)))

(defn error [& args]
  ((:error @loggers) (apply str args)))
